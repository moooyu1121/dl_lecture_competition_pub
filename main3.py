...
class VQADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for word in question_words:
            question_idx = self.question2idx.get(word, -1)
            if question_idx != -1:
                question[question_idx] = 1  # 分散表現
...
class VQAModel(nn.Module):
    def __init__(self, vocab_size: int, n_answer: int):
        ...
        self.text_encoder = nn.Embedding(vocab_size, 512)  # 分散表現に変更
    ...
    def forward(self, image, question):
        ...
        question_feature = self.text_encoder(question.long()).mean(dim=1)  # テキストの特徴量(分散表現の平均)
